...
for i in range(10):
    ...
    # display supplier rates (readonly)
    data["supplier_standing"] = cols[4].text_input(
        "Supplier Standing (p/day)",
        value=str(data.get("supplier_standing", "")),
        disabled=True,
        key=f"ss_{i}"
    )
    data["supplier_unit"] = cols[5].text_input(
        "Supplier Unit (p/kWh)",
        value=str(data.get("supplier_unit", "")),
        disabled=True,
        key=f"su_{i}"
    )

    # uplifts
    data["stand_uplift"] = cols[6].number_input(
        "Standing Uplift (p/day)",
        min_value=0.0,
        value=data.get("stand_uplift", 0.0),
        step=0.1,
        key=f"uplift_s_{i}"
    )
    data["unit_uplift"] = cols[7].number_input(
        "Unit Uplift (p/kWh)",
        min_value=0.0,
        value=data.get("unit_uplift", 0.0),
        step=0.1,
        key=f"uplift_u_{i}"
    )
    data["cost_per_metre"] = cols[8].number_input(
        "Cost/metre (p/metre)",
        min_value=0.0,
        value=data.get("cost_per_metre", 0.0),
        key=f"cpm_{i}"
    )
    ...
    data["final_standing"] = cols[9].text_input(
        "Final Standing (p/day)",
        value=str(round(final_standing, 4)) if final_standing else "",
        disabled=True,
        key=f"fs_{i}"
    )
    data["final_unit"] = cols[10].text_input(
        "Final Unit (p/kWh)",
        value=str(round(final_unit, 4)) if final_unit else "",
        disabled=True,
        key=f"fu_{i}"
    )
