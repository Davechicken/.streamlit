...
# Display logo
st.image("Dyce_logo.png", width=200)
...
*Version 1 shows a preview only — we'll add downloads next.*
""")
...
site_data = []
columns = [
    "MPRN", "Site Name", "AQ (kWh)", "Postcode", "Carbon Offset", "Cost per Metre (p/metre)"
]
st.write("Please fill in as many rows as needed:")
...
if st.button("Calculate Pricing"):
    if not tariff_df is None:
        results = []
        
        for site in site_data:
            if site["mprn"] != "":
                # attempt to match tariff based on carbon offset and AQ band
                band = tariff_df[(
