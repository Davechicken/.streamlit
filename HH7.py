...
if uploaded_file:
    ...
    if hh_df.empty:
        ...
        selected_durations = st.multiselect("Select Contract Durations (Months):", contract_options, default=contract_options)

        # Start date filter
        min_start = pd.to_datetime(hh_df["Minimum_Contract_Start_Date"]).min().date()
        max_start = pd.to_datetime(hh_df["Maximum_Contract_Start_Date"]).max().date()
        start_date = st.date_input("Select Contract Start Date Range:", (min_start, max_start))

        # Filter data accordingly
        mask = (
            hh_df["Contract_Duration"].isin(selected_durations)
            & (pd.to_datetime(hh_df["Minimum_Contract_Start_Date"]) <= pd.to_datetime(start_date[1]))
            & (pd.to_datetime(hh_df["Maximum_Contract_Start_Date"]) >= pd.to_datetime(start_date[0]))
        )
        filtered_df = hh_df[mask]
        ...
        if filtered_df.empty:
            ...
            display_cols = [
                "Consumption_Band", "Contract_Duration", "Minimum_Contract_Start_Date", "Maximum_Contract_Start_Date",
                "Green_Energy", "Standing_Charge", "Day_Rate", "Night_Rate", "Capacity_Rate"
            ]
