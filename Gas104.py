...
if uploaded_file:
    ...
    # Drop unwanted credit score columns if present
    ...
    with st.expander("Click to enter uplifts for each band", expanded=True):
        for i, band in enumerate(bands):
            ...
            # Dropdown for contract duration
            duration = cols[0].selectbox(
                "Contract Duration", ["1 Year", "2 Year", "3 Year"], key=f"duration_{i}"
            )
            ...
            entry = {
                ...
                "Duration": duration,
                # Standard
                "Standard_Standing": cols[1].number_input(
                    "Std Standing (p/day)", min_value=0, value=0, step=1, key=f"std_standing_{i}"
                ),
                "Standard_Unit": cols[2].number_input(
                    "Std Unit (p/kWh)", min_value=0.000, value=0.000, step=0.001, format="%.3f", key=f"std_unit_{i}"
                ),
                # Carbon Neutral
                "Carbon_Standing": cols[3].number_input(
                    "Carbon Standing (p/day)", min_value=0, value=0, step=1, key=f"carbon_standing_{i}"
                ),
                "Carbon_Unit": cols[4].number_input(
                    "Carbon Unit (p/kWh)", min_value=0.000, value=0.000, step=0.001, format="%.3f", key=f"carbon_unit_{i}"
                ),
            }
    ...
    def get_band_uplift(row):
        ...
        contract_duration = str(row.get("Contract_Duration", "")).strip().lower()
        # Match duration label to dropdown text
        duration_label = (
            "1 Year" if "1" in contract_duration else
            "2 Year" if "2" in contract_duration else
            "3 Year"
        )

        # Find matching band and duration
        band = next(
            (
                b for b in band_inputs
                if b["Min"] <= consumption <= b["Max"]
                and b["Duration"] == duration_label
            ),
            None
        )

        if band is None:
            return pd.Series({"Uplift_Unit": 0.0, "Uplift_Standing": 0.0})
    ...
    # Convert standing charge to pence (e.g., £1.05 → 105p)
